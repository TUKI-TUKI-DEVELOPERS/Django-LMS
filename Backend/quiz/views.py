...
from .models import Quiz, Question, Progress, Sitting, MCQuestion, EssayQuestion
from .forms import QuestionForm, EssayForm, QuizAddForm, MCQuestionFormSet
    ...
    def form_valid(self, form, **kwargs):
        ...
        with transaction.atomic():
            ...
            if form.is_valid():
                ...
                form.save()
                return redirect(
                    "quiz:mc_create", slug=self.kwargs["slug"], quiz_id=form.instance.id
                )
    ...
    def form_valid(self, form, **kwargs):
        ...
        with transaction.atomic():
            ...
            if form.is_valid():
                form.instance = self.object
                form.save()
                return redirect("quiz:quiz_index", course.slug)
...
def quiz_delete(request, slug, pk):
    ...
    quiz.delete()
    messages.success(request, f"Eliminado exitosamente.")
    return redirect("quiz:quiz_index", quiz.course.slug)


@method_decorator([login_required, lecturer_required], name="dispatch")
class MCQuestionCreate(CreateView):
    model = MCQuestion
    fields = ["content", "figure", "explanation", "choice_order"]
    ...
    def form_valid(self, form):
        ...
        if formset.is_valid():
            with transaction.atomic():
                ...
                self.object = form.save()
                # Add the quiz to the question's quiz set
...
def quiz_list(request, slug):
    ...
    course = Course.objects.get(slug=slug)
    # Identify which quizzes the current user has already completed for this course
    completed_quiz_ids = list(
        Sitting.objects.filter(user=request.user, course=course, complete=True)
        .values_list("quiz_id", flat=True)
    )
    return render(
        request,
        "quiz/quiz_list.html",
        {"quizzes": quizzes, "course": course, "completed_quiz_ids": completed_quiz_ids},
    )
...
class QuizTake(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if quizQuestions <= 0:
            messages.warning(request, f"El conjunto de preguntas del examen está vacío. ¡Inténtalo más tarde!")
            return redirect("quiz:quiz_index", self.course.slug)
        ...
        if self.sitting is False:
            ...
            messages.info(
                request,
                f"Ya has realizado este examen y solo se permite una sesión",
            )
            return redirect("quiz:quiz_index", self.course.slug)
