...
class Quiz(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, *args, **kwargs):
        ...
        if self.pass_mark > 100:
            raise ValidationError("%s está por encima de 100" % self.pass_mark)
        if self.pass_mark < 0:
            raise ValidationError("%s está por debajo de 0" % self.pass_mark)
    ...
    def get_absolute_url(self):
        ...
        return reverse("quiz:quiz_index", kwargs={"slug": self.course.slug})
    ...
    def new_sitting(self, user, quiz, course):
        ...
        if len(question_set) == 0:
            raise ImproperlyConfigured(
                _("El conjunto de preguntas del examen está vacío. Por favor, configura las preguntas correctamente")
            )
